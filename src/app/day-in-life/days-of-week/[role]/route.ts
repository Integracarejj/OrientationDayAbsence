...
// Next.js App Router: params is a Promise; unwrap with await
export async function GET(
    ...
    const url =
        `${baseUrl}/api/DaysOfWeekGet/${encodeURIComponent(role)}` +
...
export async function PUT(
    ...
    const url =
        `${baseUrl}/api/DaysOfWeekPut/${encodeURIComponent(role)}` +
        `?code=${encodeURIComponent(funcKey)}`;
